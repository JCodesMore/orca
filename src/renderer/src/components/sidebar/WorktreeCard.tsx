...
const WorktreeCard = React.memo(function WorktreeCard({
  ...
  const parentCardContent = (
    ...
    >
      {/* Why: subtle repo.badgeColor wash painted behind the card so each
          project owns a sidebar swim lane. Sits at -z-10 so the active/hover
          state overlays above composite on top without specificity fights. */}
      {repo && (
        <div
          aria-hidden
          className={cn(
          ...
          style={{ backgroundColor: getRepoTintBackground(repo.badgeColor) }}
        />
      )}
        ...
        <div className="flex min-w-0 items-center justify-between gap-2">
          <div className="flex min-w-0 flex-1 items-center gap-1.5">
            ...
            {!compactCards && worktree.isMainWorktree && !isFolder && (
              <Tooltip>
                <TooltipTrigger asChild>
                  {/* Borderless primary label — fork customization (see commit
                      "Restore borderless primary label in sidebar"). */}
